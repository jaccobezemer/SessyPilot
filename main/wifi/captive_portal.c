...
static const char PORTAL_PAGE[] =
    ...
    "<title>SessyPilot WiFi Setup</title>"
    "<style>"
    ...
    "<p class='sub'>Verbind SessyPilot met je netwerk</p>"
    "<form method='post' action='/'>"
    "<label>WiFi naam (SSID)</label>"
    "<input type='text' name='ssid' placeholder='MijnNetwerk' required>"
    "<label>Wachtwoord</label>"
    "<input type='password' name='password' placeholder='Wachtwoord (leeg = open netwerk)'>"
...
static esp_err_t portal_post_handler(httpd_req_t *req)
{
    ...
    settings_set_wifi(ssid, password);
    ...
    static const char SUCCESS_POST[] =
        "</strong>.<br>SessyPilot herstart nu...</p>"
        "</div></body></html>";
    ...
    /* Reboot after 2 seconds so response is sent first */
...
esp_err_t captive_portal_start(void)
{
    ...
    httpd_uri_t post_uri = { .uri = "/", .method = HTTP_POST, .handler = portal_post_handler };
    httpd_register_uri_handler(s_portal_server, &get_uri);
    httpd_register_uri_handler(s_portal_server, &post_uri);

    ESP_LOGI(TAG, "Captive portal started — http://192.168.4.1");
    return ESP_OK;
}
